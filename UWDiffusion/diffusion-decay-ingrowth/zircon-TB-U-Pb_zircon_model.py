...
CFL_fac      = uw.options.getReal("CFL_fac", default=1.)
...
zircon_size = uw.options.getInt(name='zircon_size', default = 1)



outputPath = uw.options.getString('outputPath', default= f'./output/TB_case_study-profile={profile}_zircon_size={zircon_size}_csize={csize}-cfl={CFL_fac}/')
...
t_points = [0, 40, 75, 90, 100, 125, 200, 1340, 1350, 1380, 1395, 1410, 1440, 1460, 1950]

t_start = t_points[0]
t_end = t_points[-1]

if profile == 1:
    T_points = [800, 790, 780, 770, 760, 725, 725, 725, 825, 850, 850, 850, 825, 725, 725]
elif profile == 2:
    T_points = [800, 790, 780, 770, 760, 725, 725, 725, 850, 900, 900, 900, 850, 725, 725]
elif profile == 3:
    T_points = [800, 790, 780, 770, 760, 725, 725, 725, 900, 950, 950, 950, 900, 725, 725]
else:
    T_points = [800, 790, 780, 770, 760, 725, 725, 725, 900, 1000, 1000, 1000, 900, 725, 725]
...
# %%
time_profile = np.linspace(t_start, t_end, 500 ) ### every ~4 Myr
# # temp_profile = T_func(time_profile)
temperature_interp = interp1d(t_points, T_points, kind='slinear')
temp_profile = gaussian_filter(temperature_interp(time_profile), sigma=2)
...
if uw.mpi.rank == 0:
    ...
    plt.savefig(f'{outputPath}/temperature_profile.pdf', bbox_inches='tight')
...
if zircon_size == 1:
    points = [(-35, 20, 0), (-20, 35, 0), (20, 35, 0), (35, 20, 0), 
            (35, -20, 0), (20, -35, 0), (-20, -35, 0), (-35, -20, 0)]
elif zircon_size == 2:
    points = [(-40, 35, 0), (-25, 60, 0), (25, 60, 0), (40, 35, 0), 
           (40, -35, 0), (25, -60, 0), (-25, -60, 0), (-40, -35, 0)]
elif zircon_size == 3:
    points = [(-45, 40, 0), (-30, 80, 0), (30, 80, 0), (45, 40, 0), 
           (45, -40, 0), (30, -80, 0), (-30, -80, 0), (-45, -40, 0)]
else:
    points = [(-50, 50, 0), (-35, 100, 0), (35, 100, 0), (50, 50, 0), 
           (50, -50, 0), (35, -100, 0), (-35, -100, 0), (-50, -50, 0)]



points_array = np.array(points) / KL.m
...
# %%
U238_Pb206.run_simulation(duration=t_end*u.megayear, min_dt = 10.*u.megayear, max_dt=dt_temp_change*u.megayear, diffusion_time_step_factor=CFL_fac)
...
# %%
U235_Pb207.run_simulation(duration=t_end*u.megayear, min_dt = 10.*u.megayear, max_dt=dt_temp_change*u.megayear, diffusion_time_step_factor=CFL_fac)
...
if save_figs:
    ...
    DIF.utilities.plot_terra_wasserburg_plot(start_time=2000e6, end_time=400e6, marker_spacing=100e6, ax=ax)
    ...
    ax.scatter(spot_data[:,0], spot_data[:,1], marker='d', c='lightblue', s = 150, alpha=0.4, label='Interface')
